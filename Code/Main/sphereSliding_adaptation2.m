...
%Remesh glenoid to 0.5 point spacing
[glenoidF,glenoidV] = triRemeshLabel(glenoidF,glenoidV,0.5);
...
%Remesh sphere
[E2,V2] = triRemeshLabel(E2,V2,0.5);

%Shift sphere by a contact offset
contactInitialOffset = 0.5;
minV2 = -24;
V2(:,3) = V2(:,3)-minV2+contactInitialOffset;

center_of_mass = mean(V2,1);

cFigure; hold on;
gpatch(E2,V2,'kw','k'); 
gpatch(glenoidF,glenoidVolV,'kw','k');
axisGeom;
...
glenoidVolFb = fliplr(glenoidVolFb); %needs to be flipped for some reason
...
stepStruct.Control.step_size=0.01;
stepStruct.Control.time_stepper.dtmin=(1/100)/100;
stepStruct.Control.time_stepper.dtmax=1/100; 
...
febio_spec.Step{1}.Boundary.rigid_body{1}.prescribed.VAL=-1;
...
pointSpacings = 0.5; %I think this is spacing of glenoid
switch contactType
    case 'sticky'
    ...
    case 'facet-to-facet-sliding'
        ...
        febio_spec.Contact.contact{1}.auto_penalty=1; %changed
        febio_spec.Contact.contact{1}.two_pass=0;
        febio_spec.Contact.contact{1}.laugon=0;
        febio_spec.Contact.contact{1}.tolerance=0.1;
        febio_spec.Contact.contact{1}.gaptol=0; %changed for laugon
        febio_spec.Contact.contact{1}.minaug=0;
        febio_spec.Contact.contact{1}.maxaug=10;
...
febio_spec.LoadData.loadcurve{1}.point.VAL=[0 0; 0.1 1];
